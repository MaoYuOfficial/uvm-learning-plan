...
initial begin//主激励：发 3 帧、每帧收完自动比对、最后出总结
    #100 rst_n = 1;
    #100 expected_byte = 8'h55;//令期望的数据为8'h55
    send_byte(8'h55);//调用 task 发一帧 0x55
    @(posedge utop_uart_top.rx_done);//等 RX 收完本帧（收卷时机），再读 rx_data 比对
    if (utop_uart_top.rx_data == expected_byte) begin//相同 → 报 PASS 并计数
        $display("PASS: exp 0x%02x == got 0x%02x", expected_byte, utop_uart_top.rx_data);
        pass_cnt = pass_cnt + 1;
    end

    else begin//不同 → 报 FAIL 并计数
        $display("FAIL: exp 0x%02x != got 0x%02x", expected_byte, utop_uart_top.rx_data);
        err_cnt = err_cnt + 1 ;
    end
    ...
    expected_byte = 8'hAA;//第二帧数据传输，令期望的数据为8'hAA
    ...
    expected_byte = 8'h3C;//第三帧同理
    send_byte(8'h3C);
    @(posedge utop_uart_top.rx_done);
    if (utop_uart_top.rx_data == expected_byte) begin
        $display("PASS: exp 0x%02x == got 0x%02x", expected_byte, utop_uart_top.rx_data);
        pass_cnt = pass_cnt + 1;
    end
